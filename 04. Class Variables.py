...
'''In the above line we can use Employee.raise_amt as well as
self.raise_amt depending on our interest but not raise_amt.'''
...
the instance. As we try to acess an attribute on an instance, it first's checks
whether the instance has that attribute or not, then it will see the class
or any class that inherits from contains that attribute.'''
